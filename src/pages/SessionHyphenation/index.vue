...
watch(
  ...
  (newData, oldData) => {
    ...
    if (newData?.length < oldData?.length) {
      currMargin.value = currMargin.value - 0.75;
    } else if (newData?.length > oldData?.length)
      currMargin.value = currMargin.value + 0.25;
  },
  { immediate: true }
);
...
<template>
  <div class="session-container w-full flex flex-col relative">
    <div
      class="max-[639px]:h-[3.375rem] h-[4rem] w-full left-0 flex items-center top-0 py-[0.25rem] px-[0.25rem] min-[640px]:px-[1.25rem] shrink-0 justify-center">
    ...
    <div v-if="!isLoading && isError && errorLog?.message !== 'Lexemes not found.'"
      ...
      <div class="w-full text-center max-w-[56rem] absolute top-[40%] px-[1rem]">
        ...
        <p class="text-[0.875rem] pb-[1rem]" style="font-family: monospace">
    ...
    <div v-else-if="!isLoading && !isError && !noInternet"
      ...
      <Transition name="undo" mode="out-in">
        <div :key="undoWarn" v-if="undoWarn" :class="[
          ...
          <div
            class="w-full max-w-[28.125rem] min-w-[18rem] bg-black dark:bg-white relative mx-[0.5rem] rounded-[0.125rem]"
            style="box-shadow: var(--box-shadow-large)">
            <v-progress-linear v-if="!isPreferredMotion" v-model="progress.number" color="#3366CC"
              class="absolute rounded-t-[0.125rem]"></v-progress-linear>
    ...
    <div v-if="!isLoading && !isError && !noInternet"
      class="flex bottom-0 w-full p-[0.875rem] justify-center left-0 z-[1] absolute">
