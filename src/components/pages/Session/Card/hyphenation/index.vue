  ...
  >
      ...
      <div class="flex items-center justify-between">
        <div>
          ...
          >
            ...
            >
              <h4 class="text-[18px] font-bold text-[#9F3526]">{{ item }}</h4>
            </div>
          </div>
          <span v-if="props?.data?.gloss" class="text-white text-[16px] flex"
            ><span
            ...
            ></span
          >
          <span class="text-white text-[16px]" v-else>
            <i>
        ...
        <div
          class="w-[34px] h-[34px] flex justify-center items-center cursor-pointer interactable"
    ...
    >
      ...
      <div class="w-full max-w-[450px] relative" dir="ltr">
        ...
        >
          <div
            v-for="(slide, index) in splitWithEmptySlots(word)"
            :class="[
          ...
          >
            ...
            >
              <div
                v-if="!selectedIndexes?.find((item) => item === index)"
                class="border-r-[2px] h-full border-[#C8CCD1] ml-[-1px]"
              ></div>
            </div>
          </div>
        </div>
...
.hyphenation-footer::after {
  content: "";
  position: absolute;
  top: -24px; /* Adjust this to control the cutout position */
  left: 50%;
  transform: translateX(-50%);
  width: 0;
  height: 0;
...
.hyphenation-footer::before {
  content: "";
  position: absolute;
  top: -26px; /* Slightly higher than `::after` */
  left: 50%;
  transform: translateX(-50%);
  width: 0;
  height: 0;
