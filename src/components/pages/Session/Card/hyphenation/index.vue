...
<template>
  <div class="relative w-full flex flex-col overflow-hidden flex flex-col h-full dark:bg-black rounded-[0.9375rem]">
    ...
    <div class="max-h-[40.625rem] h-100 flex items-center relative py-[1.5rem] bg-[var(--background-color-base)]"
          ...
          <div v-for="(slide, index) in splitWithEmptySlots(word)" :class="[
            ...
            <div v-else :class="[
              ...
              <div v-if="!selectedIndexes?.find((item) => item === index)"
                class="border-r-[0.125rem] h-full border-[#C8CCD1] ml-[-0.0625rem]"></div>
            </div>
          </div>
        </div>
      ...
      <div class="h-[100%] max-h-[31.25rem] bg-[#9F3526] w-[0.125rem] absolute left-[49.75%] top-[1.25rem]"></div>
    </div>
    <div dir="ltr"
...
.hyphenation-footer::after {
  ...
  border-bottom: 1.5625rem solid var(--background-color-neutral);
...
.hyphenation-footer::before {
  ...
  border-left: 1.5625rem solid transparent;
  border-right: 1.5625rem solid transparent;
  border-bottom: 1.5625rem solid var(--border-color-base);
  /* Same as border */
}

.x.mandatory-scroll-snapping {
  scroll-snap-type: x mandatory;
  scroll-padding-left: calc(50% - 1.625rem);
  padding-left: calc(50% - 1.625rem);
  padding-right: calc(50% - 1.625rem);
