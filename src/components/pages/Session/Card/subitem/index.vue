...
<template>
  <div class="relative w-full overflow-hidden flex flex-col h-full">
    <div
      class="header pt-[var(--spacing-100)] pl-[var(--spacing-50)] pb-[var(--spacing-75)] pr-[var(--spacing-100)]] rtl:pl-[var(--spacing-100)] rtl:pr-[var(--spacing-50)] text-white flex test justify-between gap-x-2 relative rounded-t-[0.875rem]"
      :style="{
        background: '#196551',
    ...
    <div class="p-[1rem] overflow-auto bg-white dark:bg-[#101418] h-full rounded-b-[1rem]">
      ...
      ">
        ...
        )" :key="index" class="border border-[var(--border-color-base)] rounded-[0.125rem] p-[0.75rem] mb-[0.75rem]"
          @mouseover="
            () => {
        ...
        <CdxLabel v-if="statements.find((item) => item?.[0] === 'translation')" class="text-[1rem] pt-[0.25rem]"
        ...
        <div v-for="(value, index) in statements.filter(
          ...
          <div class="flex gap-x-[0.75rem]">
            <div>
              <CdxLabel class="text-[1rem] pb-[0.25rem] leading-[1.25rem]">{{ value?.[1]?.data?.[0]?.language }} ({{
                value?.[1]?.data?.[0]?.code
              }})</CdxLabel>
              <p class="text-[1rem] font-normal text-[#54595D] dark:text-[#A2A9B1] pb-[0] leading-[1.375rem]">
                {{ value?.[1]?.data?.[0]?.value }}
