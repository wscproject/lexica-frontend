    ...
    <CdxDialog :open="props.open" @update:open="emit('onClose')" :use-close-button="true" class="activities"
      ...
      <div class="p-[1rem]" role="radiogroup" @keydown="keydown" tabindex="0">
        ...
        " @focus="setFocusedIndex(index)" :tabindex="focusedIndex === index ? 0 : -1" name="activity" :class="[
          'border border-[var(--border-color-base)] rounded-[0.125rem] p-[0.75rem] flex gap-x-[0.75rem] mb-[var(--spacing-50)] focused',
          selectedType === activity.type &&
          'border-[0.125rem] border-[var(--border-color-progressive--focus)] bg-[var(--background-color-progressive-subtle)] ',
        ]">
          <div :class="[
            selectedType === activity.type &&
            ...
            'w-[2.5rem] h-[2.5rem] border rounded-[0.125rem] overflow-hidden shrink-0',
          ]">
